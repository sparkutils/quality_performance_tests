...
import org.camunda.dmn.DmnEngine.Builder
...
object PerfTestUtils extends TestUtils {
  ...
  val dmnEngine = new Builder().build
  val dmn = dmnEngine.parser.parse(getClass().getClassLoader.getResourceAsStream("decisions.dmn")).
    fold(f => {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ctx = Map[String, Any]("testData" -> testData).asJava.asInstanceOf[java.util.Map[String, Object]]

    val res = dmnEngine.eval(dmn, "DQService", ctx)
    println(res)
  }
